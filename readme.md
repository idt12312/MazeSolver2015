...
### 最終的に走る経路
状態がFINIEHDのときにgetRunSequence()で最終的に走る経路が取得できる。
...
//壁情報へのアクセス
//直接書き込むと死ぬ
maze.wall[2][1]; //(1,2)の壁情報にDirection型としてアクセスできる
f(maze.wall[2][1][3]); //(1,2)のbit3(西)にアクセス
Direction walldata = maze.wall[2][1]; //(1,2)の壁情報で初期化

//歩数マップへのアクセス
//直接書き込むと死ぬ
maze.stepMap[2][1]; //(1,2)の歩数をuint8_tで取得
...
通れると思っていたのに通れないことが発覚した場合はk最短経路の計算をするから時間がかかる
